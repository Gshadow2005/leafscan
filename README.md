...
A group project for onion plant disease detection, built with a FastAPI backend and a standalone HTML frontend.
The backend wraps a trained MobileNetV2 Keras model into a REST API with a `/predict` endpoint.
...
1. `model/cv_models/model_fold_1.keras` through `model_fold_5.keras`
...
> Requires `dataset/test/` directory and `plant_disease_model.keras` in the project root.

---

## Deploy

### Render.com (Free — Singapore region)
...
| Frontend shows "Could not connect" | Make sure the FastAPI server is running on port 8000 |# LeafScan
